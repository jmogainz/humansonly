...
function MoreMenuModal({
  ...
  const [isNavigatingLeaderboard, setIsNavigatingLeaderboard] = useState(false);
  ...
  useEffect(() => {
    setIsNavigatingLeaderboard(false);
  ...
  const style = { top: `${menuPosition.top}px`, right: `${menuPosition.right}px` };
    ...
    <>
      ...
      <div className={styles.dropdown} style={style} role="menu" aria-label="Menu">
        <button 
          ...
          onClick={() => {
            setIsNavigatingLeaderboard(true);
            router.push('/leaderboard');
          }}
          disabled={isNavigatingLeaderboard}
          role="menuitem"
        >
          <span>{isNavigatingLeaderboard ? 'Loading...' : 'Leaderboards'}</span>
          {isNavigatingLeaderboard ? (
            <Spinner size={18} />
          ) : (
        ...
        {auth.isAuthenticated ? (
          ...
          <div className={styles.authSection}>
            <div className={styles.authLabel}>Sign In</div>
            <button 
              type="button" 
              className={styles.menuItem} 
              ...
              role="menuitem"
            >
              <span>{pendingProvider === 'google' ? 'Signing in...' : 'Continue with Google'}</span>
            ...
            <button 
              ...
              role="menuitem"
            >
              <span>{pendingProvider === 'apple' ? 'Signing in...' : 'Continue with Apple'}</span>
